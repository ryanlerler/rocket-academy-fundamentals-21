...
// Must update the emoji from the shuffledDeck as both players are drawing cards from that deck
const updateEmoji = function (shuffledDeck) {
...
const main = function (input) {
  if (programState === CARDCOUNT) {
    ...
    // Must update the emoji immediately after getting the shuffledDeck or otherwise, some pcCard, which are removed elements from the shuffledDeck, may not get the emoji updated
    updateEmoji(shuffledDeck);
    ...
    let myOutputValue = `PC drew: <br> ${pcCard.name} of ${pcCard.suit}. <br> <br> Player drew: <br> `;
    ...
    myOutputValue += `${playerCardsMessage}. <br> `;
